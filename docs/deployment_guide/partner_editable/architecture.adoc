...
** AWS CodePipeline to retrieve the latest code from GitHub (via AWS CodeConnections) in response to branch updates and a weekly Amazon EventBridge schedule. You can change this frequency in the AWS CloudFormation parameter `EcrCronExpression` when you create or update the stack.
